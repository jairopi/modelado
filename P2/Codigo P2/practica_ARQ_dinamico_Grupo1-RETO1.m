...
% supuestamente la tasa de transmisión ya está introducida de alguna manera
% con un valor por defecto que es R = 300000, hay que modificar eso!
...
policy = [3 5 8 1 2 4 7 8 6 5 6]' ; %inserte aquí su politica inicial (vector columna)
...
initial_Jpolicy = (I-P)\g ;%obtenga la J de la politica inicial
% Jpolicy = inv( (I-P) ) * g ;


initial_cost = initial_Jpolicy;
...
J = zeros (11, 8);
    for u = 1:8
        J(:,u) = Gu{u} + Pu{u}* initial_cost;
    end
[TJ , newpolicy] = min(J,[],2);

    if (newpolicy == policy) 
        sigue = false;
        M = iter
    else if (iter == max_iter) sigue = false;
