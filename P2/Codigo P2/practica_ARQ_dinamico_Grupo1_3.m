...
policy = [1;3;5;6;2;3;2;4;7;4;8]; %inserte aquí su politica inicial (vector columna)
...
%%Jpolicy = (eye(N)-P)\g; otra forma de 
Jpolicy = inv(eye(N)-P)*g; %obtenga la J de la politica inicial
...
if(policy == newpolicy)
    sigue = false;
elseif(max_iter == iter)
    sigue = false;
else
    %Calcular Jpolicy para newpolicy
    policy =  newpolicy;
    g = [];
P = [];
I = eye(N);
...
Jpolicy = inv(eye(N)-P)*g;
end
end
policy
iter
plot(1:N,initial_cost)
hold on
plot(1:N,Jpolicy,'r')
hold off
